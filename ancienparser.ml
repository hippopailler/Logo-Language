  ...
  | line :: rest ->
    ...
    if line = "" then parse_lines rest
      ...
      | condition :: _ ->
          ...
          (* Important : On augmente la profondeur ici car on a trouvé un '[' *)
          let body, rest' = parse_repeat_body (rest) in
          If (cond, body) :: parse_lines rest'
      | [] -> failwith "Invalid if syntax: missing condition"
